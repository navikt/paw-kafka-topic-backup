...
use paw_kafka_topic_backup::metrics::{get_kafka_messages_processed_count, init_metrics};
...
async fn test_kafka_messages_processed_counter() {
    ...
    let _ = insert_hwm(&mut tx, "metrics-test-topic", 0, 50)
        .await
    ...
    prosesser_melding(pool.clone(), test_message_1)
        .await
    ...
    assert_eq!(
        count_above_after_first - initial_count_above,
        1.0,
        "above_hwm counter should increase by 1 after first message"
    );
    assert_eq!(
        count_below_after_first - initial_count_below,
        0.0,
        "below_hwm counter should remain unchanged"
    );

    // Process second message
    let test_message_2 = create_test_kafka_message("metrics-test-topic", 0, 200);
    prosesser_melding(pool.clone(), test_message_2)
        .await
    ...
    assert_eq!(
        count_above_after_second - initial_count_above,
        2.0,
        "above_hwm counter should increase by 2 after second message"
    );
    assert_eq!(
        count_below_after_second - initial_count_below,
        0.0,
        "below_hwm counter should remain unchanged"
    );
}
...
async fn test_counter_not_incremented_for_skipped_messages() {
    ...
    let mut tx = pool.begin().await.expect("Failed to start transaction");
    let _ = insert_hwm(&mut tx, "skip-test-topic", 0, 100)
        .await
        .expect("Failed to insert initial HWM");
    tx.commit().await.expect("Failed to commit initial HWM");
    ...
    assert_eq!(
        count_above_after_skip - initial_count_above,
        0.0,
        "above_hwm counter should not increase for skipped messages"
    );
    assert_eq!(
        count_below_after_skip - initial_count_below,
        1.0,
        "below_hwm counter should increase for skipped messages"
    );
}
