...
mod hwm_sql_tests {
    ...
    async fn test_insert_hwm_data() {
        ...
        assert!(result.is_ok(), "Failed to insert HWM data: {:?}", result.err());
        ...
        let mut tx = pool.begin().await.expect("Failed to start transaction");
        let retrieved_hwm = get_hwm(&mut tx, &topic, partition).await
    ...
    async fn test_get_nonexistent_hwm() {
        ...
        let mut tx = pool.begin().await.expect("Failed to start transaction");
        let result = get_hwm(&mut tx, &"nonexistent-topic".to_string(), 999i32).await
            .expect("Failed to query HWM data");
        tx.commit().await.expect("Failed to commit transaction");
    ...
    async fn test_update_hwm_data() {
        ...
        // Insert initial data
        let mut tx = pool.begin().await.expect("Failed to start transaction");
        insert_hwm(&mut tx, topic.clone(), partition, initial_hwm).await
        ...
        assert!(update_result.is_ok(), "Failed to update HWM: {:?}", update_result.err());
        assert_eq!(update_result.unwrap(), true, "Update should return true when rows are affected");

        // Verify the update
        let mut tx = pool.begin().await.expect("Failed to start transaction");
        let actual_hwm = get_hwm(&mut tx, &topic, partition).await
    ...
    async fn test_update_hwm_no_change_when_lower() {
        ...
        assert_eq!(update_result.unwrap(), false, "Should return false when no rows are affected");
        ...
        assert_eq!(actual_hwm.unwrap(), initial_hwm, "HWM should remain unchanged");
    }
    ...
    async fn test_multiple_topics_and_partitions() {
        ...
        for (topic, partition, hwm) in &test_data {
            let mut tx = pool.begin().await.expect("Failed to start transaction");
            insert_hwm(&mut tx, topic.clone(), *partition, *hwm).await
        ...
        for (topic, partition, expected_hwm) in test_data {
            ...
            assert_eq!(actual_hwm.unwrap(), expected_hwm, 
                "HWM mismatch for topic {} partition {}", topic, partition);
        }
    }
    ...
    async fn test_transaction_rollback() {
        ...
        let mut tx = pool.begin().await.expect("Failed to start transaction");
        let result = get_hwm(&mut tx, &topic, partition).await
            .expect("Failed to query HWM data");
        tx.commit().await.expect("Failed to commit transaction");
