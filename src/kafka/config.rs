...
fn get_kafka_config(
    ...
    config
        ...
        .set("fetch.max.bytes", "65536")          // 64KB max fetch per request
        .set("fetch.message.max.bytes", "32768")  // 32KB max per partition (correct property name)
        ...
        .set("receive.message.max.bytes", "100000") // 100KB max response size
