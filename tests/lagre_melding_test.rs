...
async fn test_lagre_melding_i_db_new_message() {
    ...
    let _ = insert_hwm(&mut tx, "test-topic".to_string(), 0, 50).await
    ...
    let mut tx = pool.begin().await.expect("Failed to start transaction");
    let hwm = get_hwm(&mut tx, &"test-topic".to_string(), 0).await
        .expect("Failed to get HWM");
    assert_eq!(hwm, Some(100), "HWM should be updated to new offset");

    // Verify data was inserted
    let count: (i64,) = sqlx::query_as("SELECT COUNT(*) FROM data_v2 WHERE kafka_topic = $1 AND kafka_partition = $2")
        .bind("test-topic")
        .bind(0i32)
        .fetch_one(&mut *tx)
        .await
...
async fn test_lagre_melding_i_db_duplicate_message() {
    ...
    let mut tx = pool.begin().await.expect("Failed to start transaction");
    let _ = insert_hwm(&mut tx, "test-topic".to_string(), 0, 100).await
        .expect("Failed to insert initial HWM");
    tx.commit().await.expect("Failed to commit initial HWM");
    ...
    assert_eq!(count.0, 0, "Should not have inserted any data records for duplicate offset");
}
...
async fn test_lagre_melding_i_db_lower_offset_message() {
    ...
    let mut tx = pool.begin().await.expect("Failed to start transaction");
    let _ = insert_hwm(&mut tx, "test-topic".to_string(), 0, higher_hwm).await
        .expect("Failed to insert initial HWM");
    tx.commit().await.expect("Failed to commit initial HWM");
    ...
    assert_eq!(count.0, 0, "Should not have inserted any data records for lower offset");
}